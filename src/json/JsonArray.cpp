...
std::string JsonArray::to_json(const JsonArray* p_array,
  ...
  const uint32_t size = p_array->get_size();
...
bool JsonArray::remove_item(const uint32_t p_index) {
...
void JsonArray::set_item_null(const uint32_t p_index) {
  set_item(p_index, JsonFactory::CreateNull());
}

void JsonArray::set_item_bool(const uint32_t p_index, const bool p_value) {
  set_item(p_index, JsonFactory::CreateBool(p_value));
}

void JsonArray::set_item_int64(const uint32_t p_index, const int64_t p_value) {
  set_item(p_index, JsonFactory::CreateInt64(p_value));
}

void JsonArray::set_item_uint64(const uint32_t p_index, const uint64_t p_value) {
  set_item(p_index, JsonFactory::CreateUint64(p_value));
}

void JsonArray::set_item_double(const uint32_t p_index, const double p_value) {
  set_item(p_index, JsonFactory::CreateDouble(p_value));
}

void JsonArray::set_item_string(const uint32_t p_index, const Red::CString& p_value) {
...
void JsonArray::add_item_bool(const bool p_item) {
  items.PushBack(JsonFactory::CreateBool(p_item));
}

void JsonArray::add_item_int64(const int64_t p_item) {
  items.PushBack(JsonFactory::CreateInt64(p_item));
}

void JsonArray::add_item_uint64(const uint64_t p_item) {
  items.PushBack(JsonFactory::CreateUint64(p_item));
}

void JsonArray::add_item_double(const double p_item) {
...
void JsonArray::insert_item_null(const uint32_t p_index) {
  insert_item(p_index, JsonFactory::CreateNull());
}

void JsonArray::insert_item_bool(const uint32_t p_index, const bool p_item) {
  insert_item(p_index, JsonFactory::CreateBool(p_item));
}

void JsonArray::insert_item_int64(const uint32_t p_index, const int64_t p_item) {
  insert_item(p_index, JsonFactory::CreateInt64(p_item));
}

void JsonArray::insert_item_uint64(const uint32_t p_index, const uint64_t p_item) {
  insert_item(p_index, JsonFactory::CreateUint64(p_item));
}

void JsonArray::insert_item_double(const uint32_t p_index, const double p_item) {
  insert_item(p_index, JsonFactory::CreateDouble(p_item));
}

void JsonArray::insert_item_string(const uint32_t p_index,
...
Red::CString JsonArray::to_string(const Red::Optional<Red::CString>& p_indent) const {
